...
| `_migrer_index_dialogue.py` | 📐 **la migration des index pour le MODE DIALOGUE, en SIMULATION** (22/09/2026) : pour un livre déjà casté, il dit combien de morceaux la coupe sépare, combien de « beats » passeraient au **narrateur** (`--variante B`, celle qui corrige) ou resteraient au personnage (`--variante A`, l'état actuel), combien sont **laissés tranquilles** parce qu'ils sont dans une citation ouverte, et il **montre les morceaux à LIRE**. Il rappelle aussi ce qu'il ne touche **jamais** : les voix par personnage, les réglages, les verrous, les alias, la voix du narrateur — **seul le « qui parle » bouge**. `--livre 28`. **Aucune écriture** pour l'instant (elle viendra après la décision, avec copie datée) |
